...
export default function ReviewBotDashboard() {
  ...
  // Countdown logic: resets the button after 60 seconds
  useEffect(() => {
    if (cooldown > 0) {
  ...
  const handleAudit = async () => {
    ...
    try {
      ...
      // Handle the "Resource Exhausted" error gracefully
      if (res.status === 429) {
        setReport({
          status: 'ERROR',
          errors: ["Free AI limit reached. Please wait 60 seconds for the quota to reset."],
          hint: "Quota resets every minute on the Google Free Tier."
        });
        setCooldown(60); 
        setLoading(false);
        return;
      ...
      alert("Backend not responding. Is Render still waking up?");
      setReport(null);
    }
    setLoading(false);
  };
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      if (res.status === 429) {
        setCooldown(60);
        alert("Daily/Minute limit reached. Try again shortly.");
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen bg-[#0d1117] text-gray-300 p-8 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex justify-between items-center bg-[#161b22] p-3 rounded-t-lg border border-gray-800 border-b-0">
            ...
            <button 
              ...
              className={`px-6 py-1.5 rounded text-sm font-bold transition-all flex items-center gap-2 ${
                loading || cooldown > 0 ? "bg-gray-700 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-500 text-white"
              }`}
            >
        ...
        <section className="bg-[#161b22] border border-gray-800 rounded-lg p-6 flex flex-col gap-6 shadow-xl">
          ...
          {!report ? (
            <div className="flex-1 flex flex-col items-center justify-center opacity-30 italic py-20 text-center"><p>Paste your code and click Audit to begin.</p></div>
          ) : (
            <div className="space-y-6">
              <div className={`flex items-center gap-3 p-4 rounded-md border ${report.status === 'ERROR' ? 'bg-red-950/20 border-red-900/50 text-red-400' : 'bg-green-950/20 border-green-900/50 text-green-400'}`}>
                ...
                <span className="font-bold">{report.status === 'ERROR' ? "Quota/Limit Warning" : "No Bugs Detected"}</span>
              </div>
              {report.status === 'ERROR' ? (
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                  ...
                  <button onClick={handleOptimize} disabled={cooldown > 0} className={`w-full py-3 border rounded font-bold transition-all ${cooldown > 0 ? "border-gray-700 text-gray-500 cursor-not-allowed" : "border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white"}`}>
                    {cooldown > 0 ? `Wait ${cooldown}s` : "Generate Optimized Algorithm"}
                  </button>
                  {optCode && <pre className="p-4 bg-black rounded-lg text-xs overflow-x-auto text-green-400 border border-green-900/30 font-mono mt-4">{optCode}</pre>}
                </div>
              )}
            </div>
          )}
